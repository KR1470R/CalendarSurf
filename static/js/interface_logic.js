...
(()=>{
	...
	(() => {
		...
		document.addEventListener("DOMContentLoaded", () => {calendar = new Calendar();});
	})();
		...
		function requestAjax(data){
			$.ajax({
				...
				success: function (response) {
					...
					if (mode === "events"){events_btn.innerHTML = 'Simple calendar'}
				},
				error: (error) => {
					throw error
				}
		...
		function showCalendar(month, year, swiped, swiped_mode,xPos,clickedOnButton){
			...
			eventsel.addEventListener("touchend",(e)=>{
				if (eventsel.style.transform === "translateX('0px')"){
					...
					if (currentX < -400){
						next(false,currentX);setTranslateBack([eventsel])
					}else if (currentX > 400){
						back(false,currentX);setTranslateBack([eventsel])
					}else if (currentX > -400 || currentX < 400){
